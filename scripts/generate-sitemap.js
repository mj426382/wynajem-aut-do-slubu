...
const staticPages = [
  ...
  { path: '/auto-do-slubu-mazowieckie', priority: '0.9', changefreq: 'monthly', lastmod: TODAY },
  { path: '/kontakt',                   priority: '0.3', changefreq: 'yearly',  lastmod: TODAY },
  { path: '/blog',                      priority: '0.9', changefreq: 'weekly',  lastmod: TODAY },
]

// ── Dynamiczne wpisy bloga (czytane z src/blog/*.jsx) ─────────────────────────
const blogDir = join(ROOT, 'src', 'blog')
const blogFiles = readdirSync(blogDir)
  .filter(f => f.endsWith('.jsx') && f.startsWith('article'))
  .sort()
